...
public class IndexTweets {
		...
		try{
		    ...
		    System.out.println("Se ha instanciado la coleccion"); 
		      ...
		      while (it.hasNext()) {  
		    	  ...
		    	  i++;
		         Tweetl tw = new Tweetl((org.bson.Document) it.next());
		         ...
		         doc.add(new StringField("mentions", tw.getMentions(), Field.Store.YES));
	...
	public void mongoDBtoArrayTweets(String nameDB, String nameColl) throws IOException
	{
		try{
			//Idenfiticacion
			MongoClient mongoClient = new MongoClient( "localhost" , 27017); 
